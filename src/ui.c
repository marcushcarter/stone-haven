...
void render_ui (bool active) {
    if (active) {
        ...
        if (!set.hide_hud && inventory[inventory_slot].block != NULL) {
	        ...
	        text_rect(renderer, floatarr(4, 0.0f, 0.0f, (float)(win.sh2+win.sw2)/2*0.25, (float)(win.sh2+win.sw2)/2*0.25), floatarr(4, (inventory[inventory_slot].block->type)*64.0f, 0.0f, 64.0f, 64.0f), block_textures64, true); // block picked 64px
